...
@implementation ZG_SVGGradientLayer
...
- (void)renderInContext:(CGContextRef)ctx {
    ...
    if ([self.gradientElement isKindOfClass:[ZG_SVGRadialGradientElement class]]) {
        [self renderRadialGradientInContext:ctx];
    } else if ([self.gradientElement isKindOfClass:[ZG_SVGLinearGradientElement class]]) {
        [self renderLinearGradientInContext:ctx];
    }
}

- (CGGradientRef)createCGGradient {
    ZG_SVGGradientElement *gradientElement = self.gradientElement;
    if ([gradientElement isKindOfClass:[ZG_SVGRadialGradientElement class]]) {
        ZG_SVGLength *svgR = ((ZG_SVGRadialGradientElement *)gradientElement).r;
        if (svgR.value <= 0) {
            return nil;
        }
    }
...
- (void)renderLinearGradientInContext:(CGContextRef)ctx {
    ZG_SVGLinearGradientElement *gradientElement = (ZG_SVGLinearGradientElement *)self.gradientElement;
    BOOL inUserSpace = gradientElement.gradientUnits == ZG_SVG_UNIT_TYPE_USERSPACEONUSE;
    CGRect objectRect = self.objectRect;
    CGRect rectForRelativeUnits = inUserSpace ? ZG_CGRectFromSVGRect( self.viewportRect ) : objectRect;
...
-(void)renderRadialGradientInContext:(CGContextRef)ctx {
    ZG_SVGRadialGradientElement *gradientElement = (ZG_SVGRadialGradientElement *)self.gradientElement;
    BOOL inUserSpace = gradientElement.gradientUnits == ZG_SVG_UNIT_TYPE_USERSPACEONUSE;
    CGRect objectRect = self.objectRect;
    CGRect rectForRelativeUnits = inUserSpace ? ZG_CGRectFromSVGRect( self.viewportRect ) : objectRect;
