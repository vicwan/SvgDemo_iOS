...
-(CALayer*) ZG_cloneRecursively
{
	return [self ZG_cloneOptionallRecurse:TRUE];
}

-(CALayer*) ZG_cloneShallow
{
	return [self ZG_cloneOptionallRecurse:FALSE];
}

-(CALayer*) ZG_cloneOptionallRecurse:(BOOL) shouldRecurse
{
	...
	if( [clone isKindOfClass:[CALayer class]])
	{
		...
		specificClone.mask = [selfSpecific.mask ZG_cloneRecursively];
	...
	{
		[clone addSublayer:[subLayer ZG_cloneRecursively]];
	}
	}
