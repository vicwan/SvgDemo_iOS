...
@implementation ZG_SVGKImage (CGContext)

-(CGContextRef) newCGContextAutosizedToFit
{
	NSAssert( [self hasSize], @"Cannot export this image because the SVG file has infinite size. Either fix the SVG file, or set an explicit size you want it to be exported at (by calling .size = something on this ZG_SVGKImage instance");
	if( ! [self hasSize] )
		return NULL;
...
-(void) renderToContext:(CGContextRef) context antiAliased:(BOOL) shouldAntialias curveFlatnessFactor:(CGFloat) multiplyFlatness interpolationQuality:(CGInterpolationQuality) interpolationQuality flipYaxis:(BOOL) flipYaxis
{
	...
	if( ZG_SVGRectIsInitialized(self.DOMTree.viewport) )
		SVGKitLogInfo(@"[%@] DEBUG: rendering to CGContext using the current root-object's viewport (may have been overridden by user code): %@", [self class], ZG_NSStringFromSVGRect(self.DOMTree.viewport) );
	...
	if( flipYaxis )
	{
		NSAssert( [self hasSize], @"Cannot flip this image in Y because the SVG file has infinite size. Either fix the SVG file, or set an explicit size you want it to be treated as (by calling .size = something on this ZG_SVGKImage instance");
