 ...
 */
ZG_SVGKParser* ZG_getCurrentlyParsingParser()
{
...
{
	ZG_SVGKParser *parser = [[ZG_SVGKParser alloc] initWithSource:source];
...
{
	ZG_SVGKParser* parser = [self newParserWithDefaultSVGKParserExtensions:source];
	ZG_SVGKParseResult* result = [parser parseSynchronously];
...
- (id)initWithSource:(ZG_SVGKSource *) s {
	self = [super init];
	if (self) {
...
{
	ZG_SVGKParserSVG *subParserSVG = [[ZG_SVGKParserSVG alloc] init];
	ZG_SVGKParserGradient* subParserGradients = [[ZG_SVGKParserGradient alloc] init];
	ZG_SVGKParserPatternsAndGradients *subParserPatternsAndGradients = [[ZG_SVGKParserPatternsAndGradients alloc] init];
	ZG_SVGKParserStyles* subParserStyles = [[ZG_SVGKParserStyles alloc] init];
	ZG_SVGKParserDefsAndUse *subParserDefsAndUse = [[ZG_SVGKParserDefsAndUse alloc] init];
	ZG_SVGKParserDOM *subParserXMLDOM = [[ZG_SVGKParserDOM alloc] init];
...
- (ZG_SVGKParseResult*) parseSynchronously
{
	...
	self.currentParseRun = [ZG_SVGKParseResult new];
	...
	*/
	...
	if( stream == nil )
	{
		[currentParseRun addSourceError:[NSError errorWithDomain:@"ZG_SVGKit" code:2354 userInfo:@{NSLocalizedDescriptionKey : [NSString stringWithFormat:@"Source failed to create a valid NSInputStream; check your log files for why the ZG_SVGKSource failed (source = %@)",source]}]];
	...
	if (status != NSStreamStatusOpen)
	{
		...
		{
			[currentParseRun addSourceError:[NSError errorWithDomain:@"ZG_SVGKit" code:2573 userInfo:@{NSLocalizedDescriptionKey: @"The stream wouldn't open; this can happen when Apple libraries incorrectly open slowly over the internet. Any other case is probably a threading bug inside SVGKit"}]];
		}
		[stream close];
...
- (ZG_SVGKSource *)loadCSSFrom:(NSString *)href
{
    ZG_SVGKSource *cssSource = nil;
    if( [href hasPrefix:@"http"] )
    {
        NSURL *url = [NSURL URLWithString:href];
        cssSource = [ZG_SVGKSourceURL sourceFromURL:url];
    }
    else
    {
    ...
    if( cssSource == nil )
    {
        ...
        {
            cssSource = [ZG_SVGKSourceString sourceFromContentsOfString:cssText];
        }
    }
...
- (NSString *)stringFromSource:(ZG_SVGKSource *) src
{
    [src.stream open]; // if we do this, we CANNOT parse from this source again in future
    NSData *data = [NSData ZG_dataWithContentsOfStream:src.stream initialCapacity:4096 error:nil];
    ...
    if( [@"xml-stylesheet" isEqualToString:target] && ( [data rangeOfString:@"type=\"text/css\""].location != NSNotFound || [data rangeOfString:@"type="].location == NSNotFound ) )
    {
        ...
        if( startHref.location != NSNotFound )
        {
            ...
            if( startHref.location != NSNotFound )
            {
                ...
                ZG_SVGKSource* cssSource = [self loadCSSFrom:href];
                
                if( cssSource != nil )
                {
                    NSString *cssText = [self stringFromSource:cssSource];
                    ZG_CSSStyleSheet* parsedStylesheet = [[ZG_CSSStyleSheet alloc] initWithString:cssText];
...
static void processingInstructionSAX (void * ctx,
                                         ...
                                         const xmlChar * data)
{
	ZG_SVGKParser* self = ZG_getCurrentlyParsingParser();
...
- (void)handleStartElement:(NSString *)name namePrefix:(NSString*)prefix namespaceURI:(NSString*) XMLNSURI attributeObjects:(NSMutableDictionary *) attributeObjects
{
	...
	if( ! parsingRootTag && _storedChars.length > 0 )
	{
		...
		ZG_Text *tNode = [[ZG_Text alloc] initWithValue:_storedChars];
	...
	for( NSObject<SVGKParserExtension>* subParser in self.parserExtensions )
	{
		   ...
		   && [[subParser supportedTags] containsObject:name] )
		{
			...
			ZG_Node* subParserResult = [subParser handleStartElement:name document:source namePrefix:prefix namespaceURI:XMLNSURI attributes:attributeObjects parseResult:self.currentParseRun parentNode:_parentOfCurrentNode];
			
#if DEBUG_XML_PARSER
			SVGKitLogVerbose(@"[%@] tag: <%@:%@> id=%@ -- handled by subParser: %@", [self class], prefix, name, ([((ZG_Attr*)[attributeObjects objectForKey:@"id"]) value] != nil?[((ZG_Attr*)[attributeObjects objectForKey:@"id"]) value]:@"(none)"), subParser );
#endif
			...
			if( parsingRootTag )
			{
				currentParseRun.parsedDocument.rootElement = (ZG_SVGSVGElement*) subParserResult;
	...
	NSAssert( eventualParser != nil, @"Found a tag (prefix:%@ name:%@) that was rejected by all the parsers available. Perhaps you forgot to include a default parser (usually: ZG_SVGKParserDOM, which will handle any / all XML tags)", prefix, name );
	...
	/** Parser Extenstion creates a node for us */
	ZG_Node* subParserResult = [eventualParser handleStartElement:name document:source namePrefix:prefix namespaceURI:XMLNSURI attributes:attributeObjects parseResult:self.currentParseRun parentNode:_parentOfCurrentNode];
	
#if DEBUG_XML_PARSER
	SVGKitLogVerbose(@"[%@] tag: <%@:%@> id=%@ -- handled by subParser: %@", [self class], prefix, name, ([((ZG_Attr*)[attributeObjects objectForKey:@"id"]) value] != nil?[((ZG_Attr*)[attributeObjects objectForKey:@"id"]) value]:@"(none)"), eventualParser );
#endif
...
static void startElementSAX (void *ctx, const xmlChar *localname, const xmlChar *prefix,
	...
	for( ZG_Attr* newAttribute in attributeObjects.allValues )
	{
	...
	/**
	 ...
	 (I have inferred the "both" status from the definition of XML's ZG_Node class, which raises an error on setting
	 ZG_Node.prefix "if the node is an attribute, and it's in the xmlns namespace ... and ... and" -- which implies to me
	...
	for( NSString* prefix in namespacesByPrefix )
	{
		 ...
		 */
		ZG_Attr* newAttributeFromNamespaceDeclaration = [[ZG_Attr alloc] initWithNamespace:@"http://www.w3.org/2000/xmlns/" qualifiedName:[NSString stringWithFormat:@"xmlns:%@", prefix] value:namespace];
...
static void errorEncounteredSAX (void *ctx, const char *msg, ...) {
	...
	ZG_SVGKParser* self = ZG_getCurrentlyParsingParser();
	ZG_SVGKParseResult* parseResult = self.currentParseRun;
...
static NSMutableDictionary *NSDictionaryFromLibxmlAttributes (const xmlChar **attrs, int attr_ct) {
	...
	for (int i = 0; i < attr_ct * 5; i += 5) {
		...
		ZG_Attr* newAttribute = [[ZG_Attr alloc] initWithNamespace:uri qualifiedName:qname value:value];
...
+(NSDictionary *) NSDictionaryFromCSSAttributes: (ZG_Attr*) styleAttribute {
	...
	for (size_t n = 0; n <= len; n++) {
		...
		if (c == ':') {
			...
			ZG_Attr* newAttribute = [[ZG_Attr alloc] initWithNamespace:styleAttribute.namespaceURI qualifiedName:[NSString stringWithUTF8String:name] value:[NSString stringWithUTF8String:accum]];
