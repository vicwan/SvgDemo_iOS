...
@implementation ZG_CSSStyleSheet
...
-(long)insertRule:(NSString *)rule index:(unsigned long)index
{
	...
	{
		ZG_CSSStyleRule* newRule = [[ZG_CSSStyleRule alloc] initWithSelectorText:[stringSplitContainer objectAtIndex:0] styleText:[stringSplitContainer objectAtIndex:1]];
...
- (id)initWithString:(NSString*) styleSheetBody
{
    ...
    if (self)
	{
		self.cssRules = [[ZG_CSSRuleList alloc]init];
