...
@implementation ZG_CSSStyleDeclaration
	...
	for (size_t n = 0; n <= len; n++) {
		...
		if (c == ':') {
            ...
            if( accumIdx > 0 ) //if there is a ';' and '\0' to end the style, avoid adding an empty key-value pair
            {
				...
				ZG_CSSValue *cssValue;
				if( [cssValueString rangeOfString:@" "].length > 0 )
					cssValue = [[ZG_CSSValueList alloc] init];
				else
					cssValue = [[ZG_CSSPrimitiveValue alloc] init];
...
{
	ZG_CSSValue* v = [self getPropertyCSSValue:propertyName];
...
-(NSString*) item:(long) index
{
	...
	ZG_CSSValue* v = [sortedKeys objectAtIndex:index];
	return v.cssText;
}
