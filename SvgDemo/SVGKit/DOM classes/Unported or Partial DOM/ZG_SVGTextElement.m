...
@implementation ZG_SVGTextElement
...
- (CALayer *) newLayer
{
	/**
	 ...
	 And: ZG_SVGKit works by pre-baking everything into position (its faster, and avoids Apple's broken CALayer.transform property)
	 */
	CGAffineTransform textTransformAbsolute = [ZG_SVGHelperUtilities transformAbsoluteIncludingViewportForTransformableOrViewportEstablishingElement:self];
	 ...
	 */
    CGRect viewport = ZG_CGRectFromSVGRect(self.rootOfCurrentDocumentFragment.viewBox);
     ...
     */
    UIFont *font = [ZG_SVGTextElement matchedFontWithElement:self];
...
//	effectiveFontSize = CGSizeApplyAffineTransform( CGSizeMake(0,effectiveFontSize), textTransformAbsolute ).height; // NB important that we apply a transform to a "CGSize" here, so that Apple's library handles worrying about whether to ignore skew transforms etc
     ...
     */
    CGColorRef strokeColor = [ZG_SVGHelperUtilities parseStrokeForElement:self];
    CGFloat strokeWidth = 0;
    NSString* actualStrokeWidth = [self cascadedValueForStylableProperty:@"stroke-width"];
    if (actualStrokeWidth)
    {
        ZG_SVGRect r = ((ZG_SVGSVGElement*)self.viewportElement).viewport;
     ...
     */
    CGColorRef fillColor = [ZG_SVGHelperUtilities parseFillForElement:self];
    ...
    if (strokeWidth != 0 && strokeColor) {
        ...
        // A typical value for outlined text is 3.0. Actually this is not so accurate, but until we directly draw the text glyph using Core ZG_Text, we can not control the detailed stroke width follow SVG spec
    ...
    CATextLayer *label = [ZG_SVGTextLayer layer];
    [ZG_SVGHelperUtilities configureCALayer:label usingElement:self];
...
-(CALayer *) newCALayerForTextLayer:(CATextLayer *)label transformAbsolute:(CGAffineTransform)transformAbsolute
{
    ...
    if ( [actualFill hasPrefix:@"url"] )
    {
        ...
        ZG_SVGGradientLayer *gradientLayer = [ZG_SVGHelperUtilities getGradientLayerWithId:fillId forElement:self withRect:label.frame transform:transformAbsolute];
        if (gradientLayer) {
 ...
 */
+ (UIFont *)matchedFontWithElement:(ZG_SVGElement *)svgElement {
    ...
    NSArray<NSString *> *actualFontFamilies = [ZG_SVGTextElement fontFamiliesWithCSSValue:actualFamily];
    ...
    // we provide enough hint information, let Core ZG_Text using their algorithm to detect which fontName should be used
...
/**
 Convert CSS font detailed information into Core ZG_Text descriptor attributes (determine the best matched font).

 @param fontFamily fontFamily
 @param fontStyle fontStyle
 @param fontWeight fontWeight
 @param fontStretch fontStretch
 @return Core ZG_Text descriptor attributes
 */
+ (NSDictionary *)fontAttributesWithFontFamily:(NSString *)fontFamily fontStyle:(NSString *)fontStyle fontWeight:(NSString *)fontWeight fontStretch:(NSString *)fontStretch {
    ...
    // map from CSS [1, 1000] to Core ZG_Text [-1.0, 1.0], 400 represent 0.0
