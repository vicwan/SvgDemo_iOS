...
@interface ZG_SVGRadialGradientElement ()

@property (nonatomic) BOOL hasSynthesizedProperties;
@property (nonatomic) ZG_SVGLength *cx;
@property (nonatomic) ZG_SVGLength *cy;
@property (nonatomic) ZG_SVGLength *r;
@property (nonatomic) ZG_SVGLength *fx;
@property (nonatomic) ZG_SVGLength *fy;
@property (nonatomic) ZG_SVGLength *fr;

@end

@implementation ZG_SVGRadialGradientElement
...
- (ZG_SVGGradientLayer *)newGradientLayerForObjectRect:(CGRect)objectRect viewportRect:(ZG_SVGRect)viewportRect transform:(CGAffineTransform)absoluteTransform {
    ...
    ZG_SVGLength* svgCX = [ZG_SVGLength svgLengthFromNSString:cxAttr.length > 0 ? cxAttr : @"50%"];
    ZG_SVGLength* svgCY = [ZG_SVGLength svgLengthFromNSString:cyAttr.length > 0 ? cyAttr : @"50%"];
    ZG_SVGLength* svgR = [ZG_SVGLength svgLengthFromNSString:rAttr.length > 0 ? rAttr : @"50%"];
    // focal value
    ZG_SVGLength* svgFX = fxAttr.length > 0 ? [ZG_SVGLength svgLengthFromNSString:fxAttr] : svgCX;
    ZG_SVGLength* svgFY = fyAttr.length > 0 ? [ZG_SVGLength svgLengthFromNSString:fyAttr] : svgCY;
    ZG_SVGLength* svgFR = [ZG_SVGLength svgLengthFromNSString:frAttr.length > 0 ? frAttr : @"0%"];
    ...
    // Suck. When using `SVGLayredImageView`, the layer rendering is submitted to CA render server, and your custom `renderInContex:` code will not work. So we just set both built-in value (CAGradientLayer property) && custom value (ZG_SVGGradientLayer property)
...
- (void)synthesizeProperties {
    ...
    if ([gradientID length])
    {
        ...
        ZG_SVGRadialGradientElement* baseGradient = (ZG_SVGRadialGradientElement*) [self.rootOfCurrentDocumentFragment getElementById:gradientID];
