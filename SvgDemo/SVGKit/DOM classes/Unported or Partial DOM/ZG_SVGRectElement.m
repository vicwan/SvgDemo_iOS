...
@implementation ZG_SVGRectElement
...
- (void)postProcessAttributesAddingErrorsTo:(ZG_SVGKParseResult *)parseResult {
	...
	_x = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"x"]];
	
	if( [[self getAttribute:@"y"] length] > 0 )
	_y = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"y"]];
	
	if( [[self getAttribute:@"width"] length] > 0 )
	_width = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"width"]];
	
	if( [[self getAttribute:@"height"] length] > 0 )
	_height = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"height"]];
	
	if( [[self getAttribute:@"rx"] length] > 0 )
	_rx = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"rx"]];
	
	if( [[self getAttribute:@"ry"] length] > 0 )
	_ry = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"ry"]];
