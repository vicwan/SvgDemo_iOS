...
@property(nonatomic,readwrite) ZG_SVGRect requestedViewport;
@end

@implementation ZG_SVGSVGElement
...
@synthesize viewBox = _viewBox; // each ZG_SVGElement subclass that conforms to protocol "SVGFitToViewBox" has to re-synthesize this to work around bugs in Apple's Objective-C 2.0 design that don't allow @properties to be extended by categories / protocols
@synthesize preserveAspectRatio; // each ZG_SVGElement subclass that conforms to protocol "SVGFitToViewBox" has to re-synthesize this to work around bugs in Apple's Objective-C 2.0 design that don't allow @properties to be extended by categories / protocols

#pragma mark - NON SPEC, violating, properties

-(void)dealloc
{
	self.viewBox = ZG_SVGRectUninitialized();
}
...
-(void)loadDefaults
{
	self.styleSheets = [[ZG_StyleSheetList alloc] init];
}
...
-(ZG_NodeList*) getIntersectionList:(ZG_SVGRect) rect referenceElement:(ZG_SVGElement*) referenceElement { NSAssert( FALSE, @"Not implemented yet" ); return nil; }
-(ZG_NodeList*) getEnclosureList:(ZG_SVGRect) rect referenceElement:(ZG_SVGElement*) referenceElement { NSAssert( FALSE, @"Not implemented yet" ); return nil; }
-(BOOL) checkIntersection:(ZG_SVGElement*) element rect:(ZG_SVGRect) rect { NSAssert( FALSE, @"Not implemented yet" ); return FALSE; }
-(BOOL) checkEnclosure:(ZG_SVGElement*) element rect:(ZG_SVGRect) rect { NSAssert( FALSE, @"Not implemented yet" ); return FALSE; }
...
{
	return [ZG_SVGLength new];
}
-(ZG_SVGAngle*) createSVGAngle { NSAssert( FALSE, @"Not implemented yet" ); return nil; }
-(ZG_SVGPoint*) createSVGPoint { NSAssert( FALSE, @"Not implemented yet" ); return nil; }
-(ZG_SVGMatrix*) createSVGMatrix { NSAssert( FALSE, @"Not implemented yet" ); return nil; }
-(ZG_SVGRect) createSVGRect
{
	ZG_SVGRect r = { 0.0, 0.0, 0.0, 0.0 };
	return r;
}
-(ZG_SVGTransform*) createSVGTransform { NSAssert( FALSE, @"Not implemented yet" ); return nil; }
-(ZG_SVGTransform*) createSVGTransformFromMatrix:(ZG_SVGMatrix*) matrix { NSAssert( FALSE, @"Not implemented yet" ); return nil; }

-(ZG_Element*) getElementById:(NSString*) elementId
{
	return [ZG_DOMHelperUtilities privateGetElementById:elementId childrenOfElement:self];
}
...
- (void)postProcessAttributesAddingErrorsTo:(ZG_SVGKParseResult *)parseResult {
	...
	/**
	 ...
	 2. Assume that our post-parse renderer code (elsewhere in the library, in ZG_SVGKImage I believe)
	...
	/**
	 ...
	 We would need to put extra (NON STANDARD) properties on ZG_SVGDocument, for the "viewport width and height",
	 ...
	 (NB: the input to this method "ZG_SVGKParseResult" has a .parsedDocument variable, that's how we'd fetch those values here
	 */
    ...
    if (pos_x == nil || pos_x.length < 1)
        ...
        self.x = [ZG_SVGLength svgLengthFromNSString:pos_x];
    
    if (pos_y == nil || pos_y.length < 1)
        self.y = 0; // i.e. undefined
    else
        self.y = [ZG_SVGLength svgLengthFromNSString:pos_y];
    
	if( stringWidth == nil || stringWidth.length < 1 )
		self.width = nil; // i.e. undefined
	else
		self.width = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"width"]];
	
	if( stringHeight == nil || stringHeight.length < 1 )
		self.height = nil; // i.e. undefined
	else
		self.height = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"height"]];
	 ...
	 */
	if( self.width.unitType == ZG_SVG_LENGTHTYPE_PERCENTAGE )
		self.width = nil;
	if( self.height.unitType == ZG_SVG_LENGTHTYPE_PERCENTAGE )
		self.height = nil;
    ...
    if( self.width != nil && self.height != nil )
        self.requestedViewport = ZG_SVGRectMake( [self.x pixelsValue], [self.y pixelsValue], [self.width pixelsValue], [self.height pixelsValue] );
    else
        self.requestedViewport = ZG_SVGRectUninitialized();
	...
	if( [[self getAttribute:@"viewBox"] length] > 0 )
	{
		...
		_viewBox = ZG_SVGRectMake([[boxElements objectAtIndex:0] floatValue], [[boxElements objectAtIndex:1] floatValue], [[boxElements objectAtIndex:2] floatValue], [[boxElements objectAtIndex:3] floatValue]);
	}
	else
	{
		self.viewBox = ZG_SVGRectUninitialized(); // VERY IMPORTANT: we MUST make it clear this was never initialized, instead of saying its 0,0,0,0 !		
	}
	
    [ZG_SVGHelperUtilities parsePreserveAspectRatioFor:self];
	...
	if( stringWidth == nil || stringWidth.length < 1 )
		...
		self.width = [ZG_SVGLength svgLengthFromNSString:[self getAttribute:@"width"]];
    // logging
    SVGKitLogVerbose(@"[%@] DEBUG INFO: set document viewBox = %@", [self class], ZG_NSStringFromSVGRect(self.viewBox));
}

- (ZG_SVGElement *)findFirstElementOfClass:(Class)classParameter {
	for (ZG_SVGElement *element in self.childNodes)
	{
...
- (CALayer *) newLayer
{
	...
	CGRect newBoundsFromSVGTag = ZG_CGRectFromSVGRect( self.viewport );
