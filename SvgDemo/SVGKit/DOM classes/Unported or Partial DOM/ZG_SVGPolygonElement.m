...
@interface ZG_SVGPolygonElement()

- (void) parseData:(NSString *)data;

@end

@implementation ZG_SVGPolygonElement
...
/*! According to SVG spec, a 'polygon' is EXACTYLY IDENTICAL to a 'path', if you prepend the letter "M", and
 ...
 So, we take the complicated parser from ZG_SVGPathElement, remove all the multi-command stuff, and just use the
 "M" command
 */
- (void)parseData:(NSString *)data
{
    ...
    ZG_SVGCurve lastCurve = [ZG_SVGKPointsAndPathsParser readMovetoDrawtoCommandGroups:commandScanner
                                                                            path:path
                                                                      relativeTo:CGPointZero
                                                                      isRelative:FALSE];
    
    [ZG_SVGKPointsAndPathsParser readCloseCommand:[NSScanner scannerWithString:@"z"]
                                          path:path
