...
CGImageRef ZG_SVGImageCGImage(UIImage *img)
{
#if SVGKIT_UIKIT
    return img.CGImage;
#else
...
@interface ZG_SVGImageElement()
@property (nonatomic, strong, readwrite) NSString *href;
@end

@implementation ZG_SVGImageElement
...
@synthesize viewBox; // each ZG_SVGElement subclass that conforms to protocol "SVGFitToViewBox" has to re-synthesize this to work around bugs in Apple's Objective-C 2.0 design that don't allow @properties to be extended by categories / protocols
...
- (CALayer *) newLayer
{
	CALayer* newLayer = [ZG_CALayerWithClipRender layer];
	
	[ZG_SVGHelperUtilities configureCALayer:newLayer usingElement:self];
	
	NSData *imageData;
	NSURL* imageURL = [NSURL URLWithString:_href];
	ZG_SVGKSource* effectiveSource = nil;
	...
	else
	{
        ...
        if (stream) {
            ...
            NSError *error = nil;
            imageData = [NSData ZG_dataWithContentsOfStream:stream initialCapacity:NSUIntegerMax error:&error];
            if( error )
    ...
    {
        ZG_SVGKImage *svg = nil;
        
        if( effectiveSource == nil )
            effectiveSource = [ZG_SVGKSourceURL sourceFromURL:imageURL];
        ...
        if( effectiveSource != nil )
        {
            ...
            if( imageData != nil )
            {
                /** NB: sources can only be used once; we've already opened the stream for the source
                 ...
                 SVG files!)
                 */
                svg = [ZG_SVGKImage imageWithSource: [ZG_SVGKSourceNSData sourceFromData:imageData URLForRelativeLinks:imageURL]];
            }
            else
            {
                svg = [ZG_SVGKImage imageWithSource: effectiveSource];
            }
        ...
        if (!image) {
            image = ZG_SVGKGetBrokenImageRepresentation();
        }
    }
	...
	if( image != nil )
	{
        ...
        if( imageData )
            self.viewBox = ZG_SVGRectMake(0, 0, image.size.width, image.size.height);
        else
            self.viewBox = ZG_SVGRectMake(0, 0, _width, _height);
        
        CGImageRef imageRef = ZG_SVGImageCGImage(image);
        ...
        if( self.preserveAspectRatio.baseVal.align != ZG_SVG_PRESERVEASPECTRATIO_NONE
           && ABS( self.aspectRatioFromWidthPerHeight - self.aspectRatioFromViewBox) > 0.00001 )
        {
            ...
            if( self.preserveAspectRatio.baseVal.meetOrSlice == ZG_SVG_MEETORSLICE_MEET )
            {
                // shrink the image to fit in the frame, preserving the aspect ratio
                frame = [self clipFrame:frame fromRatio:ratioOfRatios];
            }
            else if( self.preserveAspectRatio.baseVal.meetOrSlice == ZG_SVG_MEETORSLICE_SLICE )
            {
                // crop the image
        ...
        frame = CGRectApplyAffineTransform(frame, [ZG_SVGHelperUtilities transformAbsoluteIncludingViewportForTransformableOrViewportEstablishingElement:self]);
