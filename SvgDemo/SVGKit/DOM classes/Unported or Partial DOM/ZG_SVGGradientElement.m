...
@implementation ZG_SVGGradientElement
...
-(void)addStop:(ZG_SVGGradientStop *)gradientStop
{
    if( _stops == nil )
	{
...
- (NSArray *)colors {
    if(_colors == nil ) //these can't be determined until parsing is complete, need to update SVGGradientParser and do this on end element
    {
        ...
        for (ZG_SVGGradientStop *theStop in self.stops)
        {
            [colorBuilder addObject:(__bridge id)ZG_CGColorWithSVGColor([theStop stopColor])];
        }
...
- (NSArray *)locations {
    if(_locations == nil ) //these can't be determined until parsing is complete, need to update SVGGradientParser and do this on end element
    {
        ...
        for (int i = 0; i < self.stops.count; i++) {
            ZG_SVGGradientStop *theStop = self.stops[i];
...
- (ZG_SVG_UNIT_TYPE)gradientUnits {
...
{
	if( [self.parentNode isKindOfClass:[ZG_SVGGElement class]] )
		return [self hasAttribute:attrName] ? [self getAttribute:attrName] : [((ZG_SVGElement*)self.parentNode) getAttribute:attrName];
	else
		return [self getAttribute:attrName]; // will return blank if there was no value AND no parent value
}

- (ZG_SVGGradientLayer *)newGradientLayerForObjectRect:(CGRect)objectRect viewportRect:(ZG_SVGRect)viewportRect transform:(CGAffineTransform)transform {
    return nil;
}
