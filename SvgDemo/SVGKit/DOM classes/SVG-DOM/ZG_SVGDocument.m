...
#import "ZG_NamedNodeMap_Iterable.h" // needed for the allPrefixesByNamespace implementation

#import "ZG_SVGKDefine_Private.h"

@implementation ZG_SVGDocument
...
- (id)init
{
    self = [super initType:ZG_DOMNodeType_DOCUMENT_NODE name:@"#document"];
    if (self) {
...
-(void)setRootElement:(ZG_SVGSVGElement *)rootElement
{
...
-(void)setDocumentElement:(ZG_Element *)newDocumentElement
{
	NSAssert( [newDocumentElement isKindOfClass:[ZG_SVGSVGElement class]], @"Cannot set the documentElement property on an SVG doc unless it's of type SVGSVGDocument" );
	...
	/*! SVG spec has two variables with same name, because DOM was written to support
	 ...
	 So, we'd better keep the two variables in sync!
	 */
	self.rootElement = (ZG_SVGSVGElement*) self.documentElement;
}
...
-(NSMutableDictionary*) allPrefixesByNamespace
{
	...
	[ZG_SVGDocument accumulateNamespacesForNode:self.rootElement intoDictionary:result];
 ...
 */
+(void) accumulateNamespacesForNode:(ZG_Node*) node intoDictionary:(NSMutableDictionary*) output
{
	/**
	...
	{
		ZG_Node* namespaceDeclaration = [xmlnsNodemap objectForKey:xmlnsNodeName];
	...
	for( ZG_Node* childNode in node.childNodes )
	{
