...
@implementation ZG_SVGClipPathElement
...
- (void)postProcessAttributesAddingErrorsTo:(ZG_SVGKParseResult *)parseResult {
    ...
    NSError *error = [NSError errorWithDomain:@"ZG_SVGKit" code:1 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                         @"<clipPath> found in SVG. May render incorrectly with ZG_SVGKFastImageView due to Apple bug in CALayer .mask rendering.", NSLocalizedDescriptionKey,
                                                                         nil]];
    [parseResult addParseErrorRecoverable:error];
    
    clipPathUnits = ZG_SVG_UNIT_TYPE_USERSPACEONUSE;
    ...
    if( units != nil && units.length > 0 ) {
        if( [units isEqualToString:@"userSpaceOnUse"] )
            ...
            clipPathUnits = ZG_SVG_UNIT_TYPE_OBJECTBOUNDINGBOX;
        else {
...
- (void)layoutLayer:(CALayer *)layer toMaskLayer:(CALayer *)maskThis
{
    ...
    if( self.clipPathUnits == ZG_SVG_UNIT_TYPE_USERSPACEONUSE )
