...
@implementation ZG_SVGHelperUtilities


+(CGAffineTransform) transformRelativeIncludingViewportForTransformableOrViewportEstablishingElement:(ZG_SVGElement*) transformableOrSVGSVGElement
{
	NSAssert([transformableOrSVGSVGElement conformsToProtocol:@protocol(ZG_SVGTransformable)] || [transformableOrSVGSVGElement isKindOfClass:[ZG_SVGSVGElement class]], @"Illegal argument, sent a non-SVGTransformable, non-SVGSVGElement object to a method that requires an SVGTransformable (NB: Apple's Xcode is rubbish, it should have thrown a compiler error that you even tried to do this, but it often doesn't bother). Incoming instance = %@", transformableOrSVGSVGElement );
	 ...
	 */
	if( [transformableOrSVGSVGElement conformsToProtocol:@protocol(ZG_SVGTransformable)])
	{
		currentRelativeTransform = ((ZG_SVGElement<ZG_SVGTransformable>*)transformableOrSVGSVGElement).transform;
	}
	else
	{
	...
	{
		ZG_SVGSVGElement<ZG_SVGFitToViewBox>* svgSVGElement = (ZG_SVGSVGElement<ZG_SVGFitToViewBox>*) transformableOrSVGSVGElement;
		 ...
		 */
		ZG_SVGRect frameViewBox = svgSVGElement.viewBox; // the ACTUAL viewbox (may be Uninitalized if none specified in SVG file)
		ZG_SVGRect frameActualViewport = svgSVGElement.viewport; // the ACTUAL viewport (dictated by the graphics engine; may be Uninitialized if the renderer has too little info to decide on a viewport at all!)
		ZG_SVGRect frameRequestedViewport = svgSVGElement.requestedViewport; // the default viewport requested in the SVG source file (may be Uninitialized if no svg width or height params in original source file)
		
		if( ! ZG_SVGRectIsInitialized(frameActualViewport))
		{
		...
		else
		{
			...
			ZG_SVGRect viewportForViewBoxToRelateTo;
			if( ZG_SVGRectIsInitialized( frameRequestedViewport ))
			{
			...
			if( ZG_SVGRectIsInitialized( frameViewBox ) )
			{
				 ...
				 */
				if( svgSVGElement.preserveAspectRatio.baseVal.meetOrSlice == ZG_SVG_MEETORSLICE_MEET ) // ALWAYS TRUE in current implementation
				{
					if( ABS( svgSVGElement.aspectRatioFromWidthPerHeight - svgSVGElement.aspectRatioFromViewBox) > 0.00001 )
					{
						...
						if( ratioOfRatios > 1.0 ) // if we're going to have space to either side
						{
							...
							{
								case ZG_SVG_PRESERVEASPECTRATIO_XMINYMIN:
								case ZG_SVG_PRESERVEASPECTRATIO_XMINYMID:
								case ZG_SVG_PRESERVEASPECTRATIO_XMINYMAX:
								{
									xTranslationRequired = 0.0;
								}break;
									
								case ZG_SVG_PRESERVEASPECTRATIO_XMIDYMIN:
								case ZG_SVG_PRESERVEASPECTRATIO_XMIDYMID:
								case ZG_SVG_PRESERVEASPECTRATIO_XMIDYMAX:
								{
									xTranslationRequired = ((ratioOfRatios-1.0)/2.0) * frameViewBox.width;
								}break;
									
								case ZG_SVG_PRESERVEASPECTRATIO_XMAXYMIN:
								case ZG_SVG_PRESERVEASPECTRATIO_XMAXYMID:
								case ZG_SVG_PRESERVEASPECTRATIO_XMAXYMAX:
								{
									xTranslationRequired = ((ratioOfRatios-1.0) * frameViewBox.width);
								}break;
									
								case ZG_SVG_PRESERVEASPECTRATIO_NONE:
								case ZG_SVG_PRESERVEASPECTRATIO_UNKNOWN:
								{
...
+(CGAffineTransform) transformAbsoluteIncludingViewportForTransformableOrViewportEstablishingElement:(ZG_SVGElement*) transformableOrSVGSVGElement
{
	 ...
	 */
	ZG_SVGElement* parentSVGElement = transformableOrSVGSVGElement;
	while( (parentSVGElement = (ZG_SVGElement*) parentSVGElement.parentNode) != nil )
	{
		if( [parentSVGElement conformsToProtocol:@protocol(ZG_SVGTransformable)] )
		{
		...
		if( [parentSVGElement isKindOfClass:[ZG_SVGSVGElement class]] )
		{
			parentAbsoluteTransform = [self transformAbsoluteIncludingViewportForTransformableOrViewportEstablishingElement:parentSVGElement];
			break;
		}
...
+(void) configureCALayer:(CALayer*) layer usingElement:(ZG_SVGElement*) nonStylableElement
{
	...
	if( [nonStylableElement conformsToProtocol:@protocol(ZG_SVGStylable)])
	{
		ZG_SVGElement<ZG_SVGStylable>* stylableElement = (ZG_SVGElement<ZG_SVGStylable>*) nonStylableElement;
...
+(CALayer *) newCALayerForPathBasedSVGElement:(ZG_SVGElement<ZG_SVGTransformable>*) svgElement withPath:(CGPathRef) pathRelative
{
	CAShapeLayer* _shapeLayer = [ZG_CAShapeLayerWithHitTest layer];
	...
	{
		ZG_SVGRect r = ((ZG_SVGSVGElement*) svgElement.viewportElement).viewport;
		
		strokeWidth = [[ZG_SVGLength svgLengthFromNSString:actualStrokeWidth]
	...
	CGPathRef finalPath = ZG_CGPathCreateByOffsettingPath( pathToPlaceInLayer, transformedPathBB.origin.x, transformedPathBB.origin.y );
	   ...
	   && (! [@"none" isEqualToString:actualStroke]) )
	{
		...
		if ( [actualStroke hasPrefix:@"url"] )
		{
			...
			fillLayer = [ZG_CAShapeLayerWithHitTest layer];
            ...
            if (!CGRectIsEmpty(pathBoundingBox)) {
                // apply gradient
                ZG_SVGGradientLayer *gradientLayer = [self getGradientLayerWithId:strokeId forElement:svgElement withRect:boundingBox transform:transformAbsolute];
	...
	if ( [actualFill hasPrefix:@"url"] )
	{
		...
		ZG_SVGGradientLayer *gradientLayer = [self getGradientLayerWithId:fillId forElement:svgElement withRect:fillLayer.frame
...
+ (ZG_SVGGradientLayer*)getGradientLayerWithId:(NSString*)gradId
                                forElement:(ZG_SVGElement*)svgElement
                                  withRect:(CGRect)r
                                 transform:(CGAffineTransform)transform
{
	...
	ZG_SVGGradientElement* svgGradient = (ZG_SVGGradientElement*) [svgElement.rootOfCurrentDocumentFragment getElementById:gradId];
    if (svgGradient == nil) {
        // SVG spec allows referenced gradient not exist and will use fallback color
        SVGKitLogWarn(@"This SVG shape has a URL fill (%@), but could not find an XML ZG_Node with that ID inside the DOM tree (suggests the parser failed, or the SVG file is corrupt)", gradId );
    }

	[svgGradient synthesizeProperties];
	
	ZG_SVGGradientLayer *gradientLayer = [svgGradient newGradientLayerForObjectRect:r
...
+(CGColorRef) parseFillForElement:(ZG_SVGElement *)svgElement
{
	NSString* actualFill = [svgElement cascadedValueForStylableProperty:@"fill"];
	NSString* actualFillOpacity = [svgElement cascadedValueForStylableProperty:@"fill-opacity"];
...
+(CGColorRef) parseFillForElement:(ZG_SVGElement *)svgElement fromFill:(NSString *)actualFill andOpacity:(NSString *)actualFillOpacity
{
    return [self parsePaintColorForElement:svgElement paintColor:actualFill paintOpacity:actualFillOpacity defaultColor:@"black"];
}

+(CGColorRef) parseStrokeForElement:(ZG_SVGElement *)svgElement
{
    NSString* actualStroke = [svgElement cascadedValueForStylableProperty:@"stroke"];
    NSString* actualStrokeOpacity = [svgElement cascadedValueForStylableProperty:@"stroke-opacity"];
...
+(CGColorRef) parseStrokeForElement:(ZG_SVGElement *)svgElement fromStroke:(NSString *)actualStroke andOpacity:(NSString *)actualStrokeOpacity
{
 ...
 */
+(CGColorRef)parsePaintColorForElement:(ZG_SVGElement *)svgElement paintColor:(NSString *)paintColor paintOpacity:(NSString *)paintOpacity defaultColor:(NSString *)defaultColor {
    ...
    if( actualPaintColor.length > 0 || actualPaintOpacity.length > 0 ) {
        ZG_SVGColor paintColorSVGColor;
        if (actualPaintColor.length > 0) {
            if (![actualPaintColor isEqualToString:@"none"]) {
                paintColorSVGColor = ZG_SVGColorFromString([actualPaintColor UTF8String]); // have to use the intermediate of an SVGColor so that we can over-ride the ALPHA component in next line
            } else {
                return NULL;
            }
        } else {
            if (![defaultColor isEqualToString:@"none"]) {
                paintColorSVGColor = ZG_SVGColorFromString([actualPaintColor UTF8String]);
            } else {
                return NULL;
            }
        }
        ...
        colorRef = ZG_CGColorWithSVGColor(paintColorSVGColor);
    }
    else
    {
        if (![defaultColor isEqualToString:@"none"]) {
            colorRef = ZG_CGColorWithSVGColor(ZG_SVGColorFromString([defaultColor UTF8String]));
        } else {
            return NULL;
        }
    }
...
+(void) parsePreserveAspectRatioFor:(ZG_Element<ZG_SVGFitToViewBox>*) element
{
    element.preserveAspectRatio = [ZG_SVGAnimatedPreserveAspectRatio new]; // automatically sets defaults
    ...
    if( stringPreserveAspectRatio.length > 0 )
    {
        ...
        for( NSString* aspectRatioCommand in aspectRatioCommands )
        {
            if( [aspectRatioCommand isEqualToString:@"meet"]) /** NB this is default anyway. Dont technically need to set it */
                element.preserveAspectRatio.baseVal.meetOrSlice = ZG_SVG_MEETORSLICE_MEET;
            else if( [aspectRatioCommand isEqualToString:@"slice"])
                element.preserveAspectRatio.baseVal.meetOrSlice = ZG_SVG_MEETORSLICE_SLICE;
            
            else if( [aspectRatioCommand isEqualToString:@"xMinYMin"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMINYMIN;
            else if( [aspectRatioCommand isEqualToString:@"xMinYMid"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMINYMID;
            else if( [aspectRatioCommand isEqualToString:@"xMinYMax"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMINYMAX;
            
            else if( [aspectRatioCommand isEqualToString:@"xMidYMin"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMIDYMIN;
            else if( [aspectRatioCommand isEqualToString:@"xMidYMid"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMIDYMID;
            else if( [aspectRatioCommand isEqualToString:@"xMidYMax"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMIDYMAX;
            
            else if( [aspectRatioCommand isEqualToString:@"xMaxYMin"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMAXYMIN;
            else if( [aspectRatioCommand isEqualToString:@"xMaxYMid"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMAXYMID;
            else if( [aspectRatioCommand isEqualToString:@"xMaxYMax"])
                element.preserveAspectRatio.baseVal.align = ZG_SVG_PRESERVEASPECTRATIO_XMAXYMAX;
