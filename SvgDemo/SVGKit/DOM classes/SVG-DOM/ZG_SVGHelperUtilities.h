...
@interface ZG_SVGHelperUtilities : NSObject
...
/** Some things - e.g. setting layer's Opacity - have to be done for pretty much EVERY ZG_SVGElement; this method automatically looks
 at the incoming element, uses the protocols that element has (e.g. SVGStylable) to automatically adapt the layer.
 
 This allows each ZG_SVGElement subclass to create a custom CALayer as needed (e.g. CATextLayer for text elements), but share the setup
 code.
...
+(CALayer *) newCALayerForPathBasedSVGElement:(ZG_SVGElement*) svgElement withPath:(CGPathRef) path;
