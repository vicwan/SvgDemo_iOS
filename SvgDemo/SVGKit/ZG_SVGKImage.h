/*
 ...
 The main class in ZG_SVGKit - this is the one you'll normally interact with
 
 c.f. ZG_SVGKit.h for more info on using ZG_SVGKit
 
 What is an SVGKImage?
 
 An ZG_SVGKImage is as close to "the SVG version of a UIImage" as we could possibly get. We cannot
 ...
 Creating an ZG_SVGKImage:
 
 - PREFERRED: use the "imageNamed:" method
 - CUSTOM ZG_SVGKSource class: use the "initWithSource:" method
 - CUSTOM PARSING: Parse using ZG_SVGKParser, then send the parse-result to "initWithParsedSVG:"
 
 
 Data:
 - UIImage: not supported yet: will be a cached UIImage that is re-generated on demand. Will enable us to implement an ZG_SVGKImageView
 that works as a drop-in replacement for UIImageView
 
 - DOMTree: the SVG DOM spec, the root element of a tree of ZG_SVGElement subclasses
 ...
 - svgWidth: the internal ZG_SVGLength used to generate the correct .size
 - svgHeight: the internal ZG_SVGLength used to generate the correct .size
 - rootElement: the ZG_SVGSVGElement instance that is the root of the parse SVG tree. Use this to access the full SVG document
 
 */

#import "ZG_SVGKDefine.h"
#import "ZG_SVGKParser.h" // needed for asynchronous loading method-signature

@class ZG_SVGDocument;
@class ZG_SVGSVGElement;
@class ZG_SVGKSource;
@class ZG_SVGKParseResult;
...
@class ZG_SVGDefsElement;

@class ZG_SVGKImage; // needed for typedef below
typedef void (^SVGKImageAsynchronousLoadingDelegate)(ZG_SVGKImage* loadedImage, ZG_SVGKParseResult* parseResult );

@interface ZG_SVGKImage : NSObject // doesn't extend UIImage because Apple made UIImage immutable
{
...
@property (nonatomic, strong, readonly) ZG_SVGKParseResult* parseErrorsAndWarnings;

@property (nonatomic, strong, readonly) ZG_SVGDocument* DOMDocument;
@property (nonatomic, strong, readonly) ZG_SVGSVGElement* DOMTree; // needs renaming + (possibly) replacing by DOMDocument
...
/**
 ...
 As of ZG_SVGKit 1.2.0, this method:
 ...
 - Creates an ZG_SVGKSource so that you can later inspect exactly where it found the file
 */
...
/**
 ...
 Returns an ZG_SVGKParser object that you can cancel, or inspect for progress (using parser.currentParseRun)
...
+ (ZG_SVGKImage *)imageWithContentsOfFile:(NSString *)path;
...
/**
 ...
 this method and ADDITIONALLY wrap your data into an ZG_SVGKSource.
 ...
 */
+(ZG_SVGKImage*) imageWithData:(NSData *)newNSData; // if you have custom source's you want to use
...
/**
 ...
 the file came from: e.g. they cannot process relative links, cross-references, etc.
 */
+(ZG_SVGKImage*) imageWithSource:(ZG_SVGKSource *)newSource; // if you have custom source's you want to use
...
/** NB: if an SVG defines no limits to itself - neither a viewbox, nor an <svg width=""> nor an <svg height=""> - and
 you have not explicitly given the ZG_SVGKImage instance a "user defined size" (by setting .size) ... then there is NO
...
/**
 ...
 Either way, you should also do:
    ...
    2. set an explicit ZG_SVGKImage.size = "..."
...
/*! If you want to provide a custom ZG_SVGKSource */
- (id)initWithSource:(ZG_SVGKSource *)source;

/*! If you already have a parsed SVG, and just want to upgrade it to an ZG_SVGKImage
