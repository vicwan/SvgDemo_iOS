...
#import "ZG_SVGSVGElement_Mutable.h" // so that changing .size can change the SVG's .viewport

#import "ZG_SVGKParserSVG.h"

#import "ZG_SVGKSourceLocalFile.h" // for convenience constructors that load from filename
#import "ZG_SVGKSourceURL.h" // for convenience constructors that load from URL as string
#import "ZG_SVGKSourceNSData.h" // for convenience constructors that load from raw incoming NSData

#import "CALayer+RecursiveClone.h"
#if SVGKIT_MAC
#import "ZG_SVGKExporterNSImage.h" // needed for .NSImage property
#else
#import "ZG_SVGKExporterUIImage.h" // needed for .UIImage property
#endif

#import "ZG_SVGKDefine_Private.h"

#if ENABLE_GLOBAL_IMAGE_CACHE_FOR_SVGKIMAGE_IMAGE_NAMED
@interface ZG_SVGKImageCacheLine : NSObject
@property(nonatomic) int numberOfInstances;
@property(nonatomic,strong) ZG_SVGKImage* mainInstance;
@end
@implementation ZG_SVGKImageCacheLine
@synthesize numberOfInstances;
@synthesize mainInstance;
@end
#endif

@interface ZG_SVGKImage ()

@property(nonatomic) CGSize internalSizeThatWasSetExplicitlyByUser;

@property (nonatomic, strong, readwrite) ZG_SVGKParseResult* parseErrorsAndWarnings;

@property (nonatomic, strong, readwrite) ZG_SVGKSource* source;

@property (nonatomic, strong, readwrite) ZG_SVGDocument* DOMDocument;
@property (nonatomic, strong, readwrite) ZG_SVGSVGElement* DOMTree; // needs renaming + (possibly) replacing by DOMDocument
...
+(void) initialize
{
	if( self == [ZG_SVGKImage class]) // Have to protect against subclasses ADDITIONALLY calling this, as a "[super initialize] line
	{
#if SVGKIT_UIKIT
...
+ (ZG_SVGKImage *)imageNamed:(NSString *)name
{
    return [self imageNamed:name inBundle:[NSBundle mainBundle] withCacheKey:@""];
}

+ (ZG_SVGKImage *)imageNamed:(NSString *)name withCacheKey:(NSString *)key
{
    return [self imageNamed:name inBundle:[NSBundle mainBundle] withCacheKey:key];
}

+ (ZG_SVGKImage *)imageNamed:(NSString *)name inBundle:(NSBundle *)bundle
{
     return [self imageNamed:name inBundle:bundle withCacheKey:@""];
}

+ (ZG_SVGKImage *)imageNamed:(NSString *)name inBundle:(NSBundle *)bundle withCacheKey:(NSString *)key
{	
#if ENABLE_GLOBAL_IMAGE_CACHE_FOR_SVGKIMAGE_IMAGE_NAMED
    ...
    ZG_SVGKImageCacheLine* cacheLine = [globalSVGKImageCache valueForKey:cacheName];
    if( cacheLine != nil )
    {
    ...
    ZG_SVGKSource *source = [ZG_SVGKSourceLocalFile internalSourceAnywhereInBundle:bundle usingName:name];
	
	/**
	 Key moment: init and parse the ZG_SVGKImage
	 */
	ZG_SVGKImage* result = [self imageWithSource:source];
    ...
    ZG_SVGKImageCacheLine* newCacheLine = [[ZG_SVGKImageCacheLine alloc] init];
...
+(ZG_SVGKParser *) imageAsynchronouslyNamed:(NSString *)name onCompletion:(SVGKImageAsynchronousLoadingDelegate)blockCompleted
{
	return [self imageWithSource:[ZG_SVGKSourceLocalFile internalSourceAnywhereInBundleUsingName:name] onCompletion:blockCompleted];
}

+(ZG_SVGKParser *) imageWithSource:(ZG_SVGKSource *)source onCompletion:(SVGKImageAsynchronousLoadingDelegate)blockCompleted
{	
#if ENABLE_GLOBAL_IMAGE_CACHE_FOR_SVGKIMAGE_IMAGE_NAMED
    if( globalSVGKImageCache == nil )
    {
    ...
    ZG_SVGKImageCacheLine* cacheLine = [globalSVGKImageCache valueForKey:source.keyForAppleDictionaries];
    if( cacheLine != nil )
    {
        cacheLine.numberOfInstances ++;
	...
	ZG_SVGKParser* parser = [ZG_SVGKParser newParserWithDefaultSVGKParserExtensions:source];
	dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
				   ^{
					   ZG_SVGKParseResult* parsedSVG = [parser parseSynchronously];
					   
					   ZG_SVGKImage* finalImage = [[ZG_SVGKImage alloc] initWithParsedSVG:parsedSVG fromSource:source];
...
+ (ZG_SVGKImage*) imageWithContentsOfURL:(NSURL *)url {
...
+ (ZG_SVGKImage*) imageWithContentsOfFile:(NSString *)aPath {
    @synchronized(self) {
	return [[[self class] alloc] initWithContentsOfFile:aPath];
    }
}

+ (ZG_SVGKParser*) imageParserWithContentsOfFileAsynchronously:(NSString *)aPath onCompletion:(SVGKImageAsynchronousLoadingDelegate)blockCompleted {
    return [self imageWithSource:[ZG_SVGKSourceLocalFile sourceFromFilename:aPath] onCompletion:blockCompleted];
}

+ (ZG_SVGKImage*) imageWithContentsOfFileAsynchronously:(NSString *)aPath onCompletion:(SVGKImageAsynchronousLoadingDelegate)blockCompleted {

    __block ZG_SVGKImage *image;
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);

    [self imageWithSource:[ZG_SVGKSourceLocalFile sourceFromFilename:aPath] onCompletion:^(ZG_SVGKImage *loadedImage, ZG_SVGKParseResult *parseResult) {
        image = loadedImage;
...
+ (ZG_SVGKImage*) imageWithSource:(ZG_SVGKSource *)newSource
{
...
+ (ZG_SVGKImage*) imageWithData:(NSData *)newNSData
{
...
+ (ZG_SVGKParser*) imageParserWithDataAsynchronously:(NSData *)newNSData onCompletion:(SVGKImageAsynchronousLoadingDelegate)blockCompleted {
    NSParameterAssert(newNSData != nil);
    SVGKitLogWarn(@"Creating an SVG from raw data; this is not recommended: SVG requires knowledge of at least the URL where it came from (as it can contain relative file-links internally). You should use the method [ZG_SVGKImage initWithSource:] instead and specify an ZG_SVGKSource with more detail" );

    return [self imageWithSource:[ZG_SVGKSourceNSData sourceFromData:newNSData URLForRelativeLinks:nil] onCompletion:blockCompleted];
}

+ (ZG_SVGKImage*) imageWithDataAsynchronously:(NSData *)newNSData onCompletion:(SVGKImageAsynchronousLoadingDelegate)blockCompleted
{
	NSParameterAssert(newNSData != nil);
	SVGKitLogWarn(@"Creating an SVG from raw data; this is not recommended: SVG requires knowledge of at least the URL where it came from (as it can contain relative file-links internally). You should use the method [ZG_SVGKImage initWithSource:] instead and specify an ZG_SVGKSource with more detail" );
    ...
    [self imageWithSource:[ZG_SVGKSourceNSData sourceFromData:newNSData URLForRelativeLinks:nil] onCompletion:^(ZG_SVGKImage *loadedImage, ZG_SVGKParseResult *parseResult) {
        image = loadedImage;
        dispatch_semaphore_signal(semaphore);
    }];
 ...
 */
- (id)initWithParsedSVG:(ZG_SVGKParseResult *)parseResult fromSource:(ZG_SVGKSource*) parseSource 
{
	self = [super init];
	if (self)
	{
		...
		{
			SVGKitLogError(@"[%@] ERROR: failed to init ZG_SVGKImage with source = %@, returning nil from init methods. Parser warnings and errors = %@", [self class], parseSource, parseErrorsAndWarnings );
			self = nil;
		}
...
- (id)initWithSource:(ZG_SVGKSource *)newSource {
	NSAssert( newSource != nil, @"Attempted to init an ZG_SVGKImage using a nil SVGKSource");
	
	self = [self initWithParsedSVG:[ZG_SVGKParser parseSourceUsingDefaultSVGKParser:newSource] fromSource:newSource];
...
- (id)initWithContentsOfFile:(NSString *)aPath {
	...
	return [self initWithSource:[ZG_SVGKSourceLocalFile sourceFromFilename:aPath]];
}

- (id)initWithContentsOfURL:(NSURL *)url {
	NSParameterAssert(url != nil);
	
	return [self initWithSource:[ZG_SVGKSourceURL sourceFromURL:url]];
}
...
- (id)initWithData:(NSData *)data
{
	...
	return [self initWithSource:[ZG_SVGKSourceNSData sourceFromData:data URLForRelativeLinks:nil]];
}
...
- (void)dealloc
{
    ...
    {
        ZG_SVGKImageCacheLine* cacheLine = [globalSVGKImageCache valueForKey:self.nameUsedToInstantiate];
        cacheLine.numberOfInstances --;
...
-(BOOL) hasSize
{
	...
	if( ZG_SVGRectIsInitialized( self.DOMTree.viewport ) )
		return true;
	
	if( ZG_SVGRectIsInitialized( self.DOMTree.viewBox ) )
		return true;
...
-(CGSize)size
{
	...
	*/
	...
	if( ! ZG_SVGRectIsInitialized( self.DOMTree.viewBox ) )
	{
		...
		effectiveViewbox = ZG_SVGRectUninitialized();
	}
	else
	...
	/* COMBINED TOGETHER: 
	 ...
	 4. otherwise ... spec is UNDEFINED. We have no viewbox, so we assume viewbox is "the bounding box of the entire SVG content, in SVG units", and use 3. above
	 */
	return ZG_CGSizeFromSVGRect( effectiveViewbox );
}
...
-(void)setSize:(CGSize)newSize
{
	...
	if( ! ZG_SVGRectIsInitialized(self.DOMTree.viewBox) && !ZG_SVGRectIsInitialized( self.DOMTree.viewport ) )
	{
		NSLog(@"[%@] WARNING: you have set an explicit image size, but your SVG file has no explicit width or height AND no viewBox. This means the image will NOT BE SCALED - either add a viewBox to your SVG source file, or add an explicit svg width and height -- or: use the .scale method on this class (ZG_SVGKImage) instead to scale by desired amount", [self class]);
	}
	
	/** "size" is part of ZG_SVGKImage, not the SVG spec; we need to update the SVG spec size too (aka the ViewPort)
	 ...
	 */
	self.DOMTree.viewport = ZG_SVGRectMake(0,0,newSize.width,newSize.height); // implicitly resizes all the internal rendering of the SVG
...
-(void)setScale:(CGFloat)newScale
{
	...
	NSAssert( ! ZG_SVGRectIsInitialized( self.DOMTree.viewBox ), @"image.scale cannot be set because your SVG has an internal viewbox. To resize this SVG, you must instead call image.size = (a new size) to force the svg to scale itself up or down as appropriate");
...
{
#if SVGKIT_MAC
	return [ZG_SVGKExporterNSImage exportAsNSImage:self antiAliased:TRUE curveFlatnessFactor:1.0f interpolationQuality:kCGInterpolationDefault]; // Apple defaults
#else
    return [ZG_SVGKExporterUIImage exportAsUIImage:self antiAliased:TRUE curveFlatnessFactor:1.0f interpolationQuality:kCGInterpolationDefault]; // Apple defaults
#endif
}
...
-(CALayer*) newCopyPositionedAbsoluteOfLayer:(CALayer *)originalLayer withSubLayers:(BOOL) recursive
{
	...
	CALayer* clonedLayer = recursive ? [originalLayer ZG_cloneRecursively] : [originalLayer ZG_cloneShallow];
...
- (CALayer *)newLayerWithElement:(ZG_SVGElement <ZG_ConverterSVGToCALayer> *)element
{
	...
	ZG_NodeList* childNodes = element.childNodes;
	ZG_Node* saveParentNode = nil;
	/**
	 Special handling for <use> tags - they have to masquerade invisibly as the node they are referring to
	 */
	if( [element isKindOfClass:[ZG_SVGUseElement class]] )
	{
		ZG_SVGUseElement* useElement = (ZG_SVGUseElement*) element;
		element = (ZG_SVGElement <ZG_ConverterSVGToCALayer> *)useElement.instanceRoot.correspondingElement;
		
		saveParentNode = element.parentNode;
		element.parentNode = useElement;

		ZG_NodeList* nodeList = [[ZG_NodeList alloc] init];
		[nodeList.internalArray addObject:element];
		childNodes = nodeList;
    }
    else
    if ( [element isKindOfClass:[ZG_SVGSwitchElement class]] )
    {
        childNodes = [(ZG_SVGSwitchElement*) element visibleChildNodes];
    }
    /**
    ...
    if ( [clipPath hasPrefix:@"url"] )
    {
        ...
        ZG_SVGClipPathElement* clipPathElement = (ZG_SVGClipPathElement*) [element.rootOfCurrentDocumentFragment getElementById:_pathId];
        NSAssert( clipPathElement != nil, @"This SVG shape has a URL clip-path (%@), but could not find an XML ZG_Node with that ID inside the DOM tree (suggests the parser failed, or the SVG file is corrupt)", _pathId );
        
        CALayer *clipLayer = [clipPathElement newLayer];
        for (ZG_SVGElement *child in clipPathElement.childNodes )
        {
            if ([child conformsToProtocol:@protocol(ZG_ConverterSVGToCALayer)]) {
                
                CALayer *sublayer = [self newLayerWithElement:(ZG_SVGElement<ZG_ConverterSVGToCALayer> *)child];
	...
	for (ZG_SVGElement *child in childNodes )
	{
		if ([child conformsToProtocol:@protocol(ZG_ConverterSVGToCALayer)]) {
	...
	/**
	 ...
	 It is not used for any other class / SVG ZG_Element.
...
{
	NSAssert( [self hasSize], @"Cannot scale this image because the SVG file has infinite size. Either fix the SVG file, or set an explicit size you want it to be exported at (by calling .size = something on this ZG_SVGKImage instance");
...
-(BOOL) isElementVisible:(ZG_SVGElement *) element
{
